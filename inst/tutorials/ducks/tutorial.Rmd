...
In your `index.qmd`, add a new code chunk with `library(tidyverse)` and `library(sf)`. Add `#| message: false` to suppress messages. Add the following to the YAML header to hide code:
...
```
<!-- HK: Expected answer shows *_files literally but real ls output shows index_files. Should show the actual generated folder name. -->
...
<!-- HK: Test chunk reads from ../../extdata/ducks/mallards.csv but students save the file to data/mallards.csv. The path shown in the expected answer doesn't match what students actually wrote in their QMD. -->
...
In `index.qmd`, update the `mallards` chunk to load the RDS file instead of re-running the pipeline. Replace the current chunk contents with a single line: `mallards <- readRDS("data/mallards.rds")`. Add `#| cache: true`. Render. In the bash Terminal, run `ls data`.
...
Create a new file `map.qmd` in your repo. Add the same YAML header and library chunk as `index.qmd`. Add a data chunk that reads `mallards <- readRDS("data/mallards.rds")` with `#| cache: true`. Render `map.qmd`. In the R Terminal, run `show_file("map.qmd", chunk = "Last")`. CP/CR.
...
Each page of a Quarto website is an independent document with its own R session — `mallards` defined in `index.qmd` is not available in `map.qmd`. The RDS approach keeps all three pages in sync: if you update `mallards.R` and re-run it, every page that calls `readRDS("data/mallards.rds")` gets the updated data on the next render, with no copy-pasting.
...
Commit `index.qmd`, `map.qmd`, and `mallards.R` with the message `"Add duck map"` and push to GitHub.
...
Mean speed peaks in August and September and again in March — the post-breeding dispersal period and the early spring return. Winter months sit near the bottom, consistent with ducks resting near the lake during cold weather. Mean speed is always pulled toward zero by the majority of stationary fixes; even at the seasonal peak, the average is well under 1 m/s because most GPS fixes still catch a resting bird.
...
This website explores two questions from that dataset: where do the ducks go,
and when do they move most? The data comes from a published study by Korner,
Sauter, Fiedler, and Jenni (2016) and is freely available through Movebank
under a Creative Commons Attribution license.
</code></pre>

<!-- HK: The expected answer only shows the intro paragraphs, not the full index.qmd file. Students' actual show_file("index.qmd") output will include the YAML header and all code chunks too, so their output won't match what's shown here. -->
...
<!-- HK: Students will likely try to commit 27+ generated files (_site/, cache folders, html files) at this point because .gitignore was not updated to exclude them earlier in the tutorial. Consider adding _site/, *_cache/, and *_files/ to .gitignore in an earlier exercise so students only commit source files here. -->
