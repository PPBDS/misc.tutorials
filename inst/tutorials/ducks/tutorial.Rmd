...
This tutorial builds a website, so Quarto will generate a lot of files you do not want in your repo: the built HTML pages, their supporting `_files` folders, the `_site` output directory, Quarto's `.quarto` working directory, and (later) render caches. Create a `.gitignore` that ignores all of them:
...
```
data  index.html  index.qmd  index_files
```
...
`quarto publish gh-pages` without a filename publishes the whole website, not just a single page. The resulting URL gives access to all three pages via the navbar. Because your `.gitignore` already excludes `_site/`, `.quarto/`, the `_files/` folders, caches, and the built HTML, the only things you commit are your source `.qmd` files, `_quarto.yml`, and the data script.
