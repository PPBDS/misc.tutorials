...
Continue to work in the same code chunk. Collect the entire `dbplyr_codes` table and print it. Because `activity_codes` is only a few hundred rows, `collect()` is fine here — it is the large tables we defer. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
Continue to work in the same code chunk. For each table in the database, count the number of rows. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
Continue to work in the same code chunk. Count activity records by major category: join `dbplyr_act` to `dbplyr_codes` to get `major_name`, count records per `major_name`, sort descending. Collect and print. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
Continue to work in the same code chunk. Compute the average `duration_min` per activity record by major category. Join `dbplyr_act` to `dbplyr_codes` to get `major_name`, average `duration_min` grouped by `major_name`, sort descending. Collect and print. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
Continue to work in the same code chunk. Count the number of respondents by `sex` and `employment_status`. Collect and print. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
Continue to work in the same code chunk. Count the number of respondents by `year`. Collect and print, arranged by year. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
