...
<!-- AR: Split to two questions. 

2 approaches to 
Just spit out the pipe, then assign to an object after. 

OR Print output - say this explicitly when we are creating a permanent object. In next question remove printed output -->
...
Now finalize the first code chunk. Edit the chunk so it contains only: the database connection, the `dbplyr_dist` lazy reference, and the two object computations from Exercise 7. 
...
<!-- AR: 1. make the object and point it out. 2. Add  -->
...
<!-- AR: once we make permanent object, create a new code chunk. -->

In a new code chunk. Print `dbplyr_resp`. Render.
...
<!-- AR: KD about collect. What is the purpose of collect()? what is meand by "defer"? -->
...
<!-- AR: Not true! What is "Data codes" in the table? -->
...
For weekday non-employed respondents, compute each person's total daily minutes in each major activity category, then average across people by `sex` and `major_name`. 
<!-- AR: 2 questions. Split here. -->
Collect and assign to `x`. Render (the first render is slow). In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
<!-- AR:
chunk 1 = download data
chunk 2 = this chunk
chunk 3 = plots -->

Now finalize this chunk. Edit the chunk so it contains only: the database connection, the three lazy table references, the `x` computation from Exercise 10, and the `y` computation from Exercise 11. Remove all exploratory code from Exercises 2–9. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
<!-- AR: show nytimes chart. Challenge students to make our chart more like this one. Beware downloading new packages. -->
