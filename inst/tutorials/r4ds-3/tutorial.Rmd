...
Record count and average duration measure different things. Traveling tops Exercise 7's count because each trip is its own record — a three-leg commute is three rows — but each trip is short, so its average duration is low. Personal Care Activities includes sleeping, which is typically one long uninterrupted record per night; that single record pulls the category's average duration far above the rest. To get total time per person per day, researchers sum `duration_min` per respondent, which Exercise 12 will do.
...
This first `summarize()` totals minutes per person per category. A respondent who watched TV in three stretches contributes one total, not three records. The next exercise averages these person-level totals across all respondents.
...
Now finalize the current working chunk. Edit the chunk so it contains only: the database connection, the three lazy table references, the `x` computation from Exercise 12, and the `y` computation from Exercise 13. Remove all exploratory code from Exercises 3–10. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
```{r time-use-19-test, eval = !is.null(con_atus)}
#| echo: true
