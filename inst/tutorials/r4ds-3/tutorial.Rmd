...
<!-- AR: Add links to article. Simplify to a single graphic, dumbbell chart is where we end. -->

<!-- AR: Sentence referring to NYTimes article -->
The Stanford Education Data Archive (SEDA), which converts each state's standardized test results into a common grade-equivalent scale so that achievement can be compared across districts and years.
...
Make a new code chunk in `analysis.qmd` and add code to connect to the database file and run `dbListTables()` to see the available tables.
...
and print it. Render. Check your Live
Server tab.
...
<!-- AR: be very clear about which code chunk we're in. -->
...
<!-- AR: make sure the table is in chronological order by year. -->
...
<!-- AR: Start by looking at the mean across all years (no filtinger). Then in the next exercise maybe look at the mean by year -->

Continue to work in the same code chunk. Filter `dbplyr_dist` to 2023 with a non-missing `rla_score`. Summarize: compute the
...
###
<!-- AR: What is this number? -->

The mean sitting slightly below zero reflects the post-COVID slide pulling most districts below the historical benchmark.
...
at 0. Give the plot a proper title, subtitle, and caption. Render.
...
Calculate for each district the positive increase in RLA score from 2015 to 2023 and print the ten districts with the largest decline. 
...
substantial loss. Often extreme values are dominated by small schools.

<!-- AR: Investigate this in next exercise? Look at the n of these schools (if available) -->

### Exercise 8

Update the chunk so it plots the distribution of `rla_change` as a histogram.
Color bars differently for positive and negative changes. Add a vertical reference line at
0. Give the plot a proper title, subtitle, and caption. Render.
...
<!-- AR: Next exercise, give AI a an image of the NYTimes plot and tell it to update our plot. -->

### Exercise 9

<!-- AR: Stick with reading alone. -->
...
print the ten districts with the largest decline. Render.
...
subtitle, and caption. Render. Check your
Live Server tab.
...
rows sorted by `rla_change`. Render. Check
...
with no y-axis label. Render. Check your
Live Server tab.
...
Add the connection and `dbListTables()` code to `analysis.qmd`. Render.
...
reference to the `activity_codes` table and assign it to `dbplyr_codes`. Render.

Then add a new chunk that prints `dbplyr_act`. Render.
...
Add a chunk that prints `dbplyr_resp`. Render.
...
tables we defer. Render. Copy and paste the
...
`dbplyr_codes`. Render. Copy and paste the
...
count. Collect and print. Render. Copy and
...
duration. Collect and print. Render. Copy
...
`dbplyr_resp`, group by both columns, count, collect, and print. Render. Copy and paste the table from the rendered HTML.
...
by `year`, count, collect, and print. Render.
...
Render (the first render is slow). Check your Live Server tab. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
That two-join, two-summarize pipeline takes real time on every render, so it earns caching. Add `#| cache: true` as the first line inside the chunk from Exercise 10. Render the QMD. Rendering with caching turned on creates an `analysis_cache/` directory next to your `analysis.qmd`, which holds the saved tibble so Quarto can reload it on subsequent renders rather than re-running the database joins each time. To confirm the directory is there, run `ls` in the **bash Terminal**. CP/CR.
...
The first render writes the cache to disk; subsequent renders load `x` from cache and skip the database joins entirely.
...
Give the chart a title, subtitle, axis labels, and a data source caption. Render.
...
Now build toward the time-of-day view shown in the NYT graphic. Add a new chunk that, for weekday non-employed respondents, computes the average minutes
...
it shows, a subtitle that names the key pattern, and a data source caption. Render.
...
Now build toward the 2024 "Americans are homebodies" finding. Add a new chunk that computes average daily leisure minutes per respondent by year,
