...
Now finalize the current working chunk. Edit the chunk so it contains only: the database connection, the three lazy table references, the `keep_cats` vector, the `x` computation from Exercise 12, and the `y` computation from Exercise 13. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
`analysis_cache/` already exists from the first cache in the US Education section — adding `#| cache: true` here writes the `x` and `y` results into it. Subsequent renders skip all five million rows of database joins.
...
```{r time-use-18-test, eval = !is.null(con_atus)}
#| echo: true
