...
Read `data/wildfires.geojson` with `read_json()` and assign it to `raw`. Print `names(raw)` and `length(raw$features)`.
...
Pull the first fire out of `raw` with `pluck()` and show its component names. Then navigate five levels deeper to extract the first coordinate pair of that fire's boundary.
...
Unnest the `features` column wider. What four columns appear?
...
Continue the pipeline: unnest `properties`. What eight fire attributes appear as flat columns?
...
Continue the pipeline: unnest `geometry` wider, using `names_sep` to avoid a naming collision. Then select the six columns you will need — `incident`, `gis_acres`, `fire_year`, `agency`, `state`, and `geometry_coordinates` — and coerce `gis_acres` to numeric and `fire_year` to integer. Assign the result to `fires`. Print `fires`.
...
Group `fires` by `fire_year`, sum the total acres burned each year, and plot the result as a line chart. Give it a proper title, axis labels, and caption.
...
Filter `fires` to the Pacific Coast states (CA, OR, WA) and re-plot the annual burned acreage. Which Pacific Coast state has the most total acres burned since 2000?
...
fires |>
  ...
  geom_line() +
  labs(
    title = "Pacific Coast Wildfire Burned Area, 1990–2023",
    subtitle = "CA, OR, WA — fires ≥ 10,000 acres",
    x = "Year",
    y = "Total acres burned",
    caption = "Source: NIFC InterAgencyFirePerimeterHistory"
  )
...
Filtering to state uses a character column, not geographic coordinates — the state code was extracted from each fire's land-management unit identifier during preprocessing. Geographic filtering can be as simple as a string match when the right metadata is present.
...
Examine the `geometry_coordinates` column for the first fire in `big_fires`. How many coordinate pairs make up its boundary? Then write two functions — one returning the mean longitude and one returning the mean latitude of all points in the first ring of a `coordinates` list — and apply them to `big_fires` with `mutate()`, creating new columns `lon` and `lat`. Print `big_fires`.
...
In a new working chunk, build a circle-marker leaflet map of `big_fires`, scaling the radius by the square root of `gis_acres`. Note what it looks like. Then replace the circle-marker code with a polygon map: read `data/wildfires.geojson` as a text string and pass it directly to leaflet using `addGeoJSON()`. Compare the two maps.
...
Read `data/imdb_snapshots.rds` into a tibble called `imdb_snapshots`. Print it. Then
add a `count(snap_year)` call to confirm the snapshot structure. Render.
...
Using `imdb_snapshots`, compare the 2015 and 2022 snapshots: pivot each film's rank from
both years into its own column, drop films that appear in only one snapshot, compute how
much each film's rank changed, and print the ten films with the largest absolute change.
...
Add the film's release decade to the rank-change data. Group by decade, compute the average
rank change per decade, and print the table sorted chronologically.
...
Filter `imdb_snapshots` to Interstellar. Pivot so each snapshot year's rank and vote count
(`number`) become their own columns. Add a `vote_growth` column showing how many times the
vote count multiplied from 2015 to 2022.
...
In a new code chunk, build a bar chart showing the average rank change from 2015 to 2022
for films grouped by their release decade. Use a rough first pass — `geom_col()` with
the decade on the y-axis and the average rank change on the x-axis — and do not assign the
plot to a variable.
