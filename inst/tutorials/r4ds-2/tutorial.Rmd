...
Ask AI to add a code chunk that fits `lm(births ~ factor(year) + factor(month) + day_of_week, data = births_tibble)`, assigns the result to `births_model`, and prints `summary(births_model)$r.squared`. In a bash terminal, run `quarto render analysis.qmd`.
...
Ask AI to add a code chunk that mutates `births_tibble` to add a column `pct_resid`, equal to the residuals from `births_model` divided by the overall mean of `births` and multiplied by 100. Assign the result to `births_adjusted`. In a bash terminal, run `quarto render analysis.qmd`.
...
Ask AI to add a code chunk that filters February 29 from `births_adjusted`, groups by `month` and `date_of_month`, computes the mean `pct_resid` for each of the 365 calendar positions, and assigns a calendar date using 2001 as a non-leap anchor year. Assign the result to `calendar_resid`. In a bash terminal, run `quarto render analysis.qmd`.
...
Ask AI to add a code chunk that plots `calendar_resid` as a line chart with `calendar_date` on the x-axis and `mean_pct_resid` on the y-axis. Add a horizontal reference line at zero and annotate at least three of the deepest holiday dips by name. In a bash terminal, run `quarto render analysis.qmd`. Check your Live Server tab.
