...
Within the `births_tibble` code chunk, add `#| cache: true` as a chunk option. Render the QMD. Rendering with caching turned on creates an `analysis_cache/` directory next to your `analysis.qmd`, which holds the parsed binary so Quarto can reload it on subsequent renders rather than re-parsing the Excel file each time. To confirm the directory is there, run `ls` in the **bash Terminal**. CP/CR.
...
`#| cache: true` is worth adding here because `read_excel()` parses a binary format on every render, which is slower than reading a plain-text CSV. The `glimpse()` in Exercise 4 showed 7,670 rows across 21 years — 365 × 21 plus 5 leap days (1996, 2000, 2004, 2008, 2012) — which confirms no calendar days are missing from the data.
...
```{r basketball-recruits-6-test, echo = TRUE}
read_excel("../../extdata/r4ds-2/nba_recruits.xlsx") |>
  mutate(
    tier = factor(
      tier,
...
```{r basketball-recruits-7-test, echo = TRUE}
read_excel("../../extdata/r4ds-2/nba_recruits.xlsx") |>
  mutate(
    tier = factor(
      tier,
...
Rename the variable from `nba_data` to `basketball_tibble`. Render. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
```{r basketball-recruits-8-test, echo = TRUE}
glimpse(basketball_tibble)
```

Compare the column types here to the `glimpse()` in Exercise 3: `recruit_group` and `tier` are now `<fct>` instead of `<chr>`.

### Exercise 9

`basketball_tibble` is the parsed result of the pipeline you wrote in Exercise 8; running that pipeline on every render is wasteful. Add `#| cache: true` as a chunk option to that pipeline. Render the QMD. In the R Terminal, run `show_file("analysis.qmd", chunk = "Last")`. CP/CR.
...
The student's chunk now has `#| cache: true` as the first line, followed by the pipeline that builds `basketball_tibble`. (No `.gitignore` step --- `analysis_cache` is already ignored from Exercise 6.)

###

`#| cache: true` saves the pipeline result to disk so subsequent renders skip re-reading and reprocessing the file.
...
```{r basketball-recruits-10-test, echo = TRUE}
basketball_tibble |>
...
```{r basketball-recruits-12-test, echo = TRUE}
basketball_tibble |>
...
```{r basketball-recruits-13-test, echo = TRUE}
basketball_tibble |>
...
```{r basketball-recruits-16-test, echo = TRUE}
basketball_tibble |>
