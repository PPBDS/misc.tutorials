...
<!-- AR: don't need to tell students about dplyr, lubridate, stringr, forcats. Keep arrow. -->

You will use packages including **[arrow](https://arrow.apache.org/docs/r/)** to work with parquet files, **[dplyr](https://dplyr.tidyverse.org/)** to join and summarize related tables, **[lubridate](https://lubridate.tidyverse.org/)** and **[stringr](https://stringr.tidyverse.org/)** to clean dates and text, and **[forcats](https://forcats.tidyverse.org/)** with **[viridis](https://sjmgarnier.github.io/viridis/)** to order and polish final charts. The tutorial has two sections: one uses cryptocurrency price, metadata, and category files to analyze market composition, and the other uses prediction-market history, market metadata, and topic labels to study forecast accuracy near resolution.
...
<!-- AR: Add a motivating plot at the beginning. -->
...
In this section you will join three parquet files — daily prices, coin metadata, and category labels — to build a line chart showing how each category's share of the total crypto market evolved from 2018 to 2026. Along the way you will use packages like **[arrow](https://arrow.apache.org/docs/r/)** to open files lazily, **[lubridate](https://lubridate.tidyverse.org/)** to parse dates, **[stringr](https://stringr.tidyverse.org/)** to clean messy fields, and **[forcats](https://forcats.tidyverse.org/)** to reorder categories for the final visualization.

### Exercise 1

<!-- AR: Need to testing these downloads. Make sure that the data we tell kids to download is available. Use this as an opportunity to learn about testing in this package. Don't need to fully download. Just make sure the files are accessible. -->
...
<!-- AR: Load 10 random rows, then we get more variety. -->

Use Arrow to do a lightweight preview of the price data: keep only observations from 2020 onward and only the fields needed for a basic market-cap analysis (`date_raw`, `coin_id`, and `market_cap_usd`). Then collect that smaller result into memory. Render.
...
Each row is one cryptocurrency on one day. For example, the first row says Aave (`aave`) had a market cap of about $1.41 billion on October 10, 2020.

###

With Arrow, filtering rows and selecting columns can happen before data moves into memory, where we can work with it using R. The `collect()` call is the step that moves the filtered data into R.
...
The `prices` table has one row per coin per day. The first row says [Aave](https://en.wikipedia.org/wiki/Aave), a cryptocurrency, on October 10, 2020 had a `market_cap_usd` of about $1.41 billion. Market cap is the total value of all circulating units of a coin at that day's price.
...
<!-- AR: split the join into 3 steps. Spread it out to show details of the joining.  -->

Our analysis needs prices, coin details, and category labels in one place.

In a new chunk, build one analysis table named `crypto` from the existing `prices`, `metadata`, and `categories` tables. Start with `prices`, join `metadata` by `coin_id`, then join `categories` by `category_id` so each daily price row has coin-level details and a human-readable category name. Both lookup tables have a `description` column; keep the coin description from `metadata` and drop the category description before joining. Print the first 5 rows instead of `categories` so you can verify the joined columns look right. Render.
...
A left join keeps every row from the main table and adds matching information from lookup tables. Here, the first join adds each coin's `category_id`, and the second join turns that ID into a readable category name.

### Exercise 8

`market_cap_str` contains human-readable values like `"$1.24T"`, `"$450.2B"`, `"$37.5M"`, and some `NA`. Add a numeric audit column, `market_cap_parsed`, that converts those display strings into dollar values by respecting the `T`, `B`, `M`, and `K` suffixes. Render.
...
Formatted financial strings need extra care because suffixes like `T`, `B`, and `M` change the scale of the number. About 5% of `market_cap_str` values are missing, which is typical of exported dashboard data and should always be checked before analysis.

### Exercise 9

<!-- AR: weird to be working with metadata here...  -->

Standardize the date fields before analysis. In `metadata`, parse the mixed-format `launch_date` values into a real date column called `launch_date_parsed`; some are written as `"Month Day, Year"` and the rest as `"YYYY-MM-DD"`. In `crypto`, parse `date_raw` into a real date column called `date`. Render.
...
Date parsers need to know the order of month, day, and year. When one column contains multiple date formats, parse each format deliberately rather than hoping one parser guesses correctly.

### Exercise 10

Clean the coin descriptions so they behave like normal text fields: replace embedded newlines (`\n`) with spaces. Confirm the fix by checking the distinct descriptions for `btc` and `eth`. Render.
...
Add a market-era label to `crypto` so later summaries can use meaningful time periods. Use the year of `date` to assign `"Bitcoin Era"` (2016 and earlier), `"ICO/Ethereum Era"` (2017–2019), `"DeFi Summer"` (2020–2021), and `"Maturation Era"` (2022 onward). Make the labels ordered chronologically, then count rows per era as a quick check. Render.
...
The `crypto` chunk is now a reusable data-preparation step. Clean it up so it keeps only the saved objects and transformations: the three table loads, the joins, and the cleaning mutations. Remove the row-count check at the bottom.
...
<!-- AR: metadata processing is in the wrong location!! just start with the prices pipe and keep working forward from there. When there's an interesting output, do some EDAs on it (hist, plot, etc...) -->
...
<!-- Do the cache on the download or the data processing -->
...
<!-- AR: not wild about this KD right here. -->

The FTX exchange token FTT fell from roughly $7.25B on November 7, 2022 to about $724M on November 9. Sudden event-driven collapses like this are often easiest to spot in a time-series plot.

### Exercise 15

<!-- AR: look at more simple plots earlier to help us get a better sense of the data. -->
...
Convert the category totals into daily market-share percentages so smaller categories are visible on a common scale. Plot category share over time as a multi-line chart. Render.
...
The share plot reveals different category trajectories that were hidden on the absolute scale. Bitcoin, stablecoins, DeFi, and meme coins each move differently, which is the answer to the tutorial's guiding question.
...
<!-- AR: Straight-laced title. Just name variables. Subtitle notes that bitcoin and smart contract platforms generally move opposite to each other. -->

```{r crypto-markets-19-test}
#| echo: true
crypto |>
  group_by(date, category_name) |>
  summarise(cat_mcap = sum(market_cap_usd, na.rm = TRUE), .groups = "drop") |>
...
UST was an algorithmic stablecoin, meaning it tried to hold a $1 value through a mechanism rather than dollar reserves. Its May 2022 collapse is a reminder that categories can contain very different kinds of assets.
...
<!-- AR: Show motivating plot at the beginning. -->
