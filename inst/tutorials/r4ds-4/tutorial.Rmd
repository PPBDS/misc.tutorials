...
crypto |>
  ...
  labs(x = NULL, y = "Share of total market cap", fill = NULL,
       title = "Crypto market cap share by category, 2018–2026",
       subtitle = "Bitcoin's share fell as smart-contract platforms and stablecoins grew",
...
`https://github.com/PPBDS/misc.tutorials/raw/refs/heads/main/inst/tutorials/r4ds-4/data/markets.parquet`
`https://github.com/PPBDS/misc.tutorials/raw/refs/heads/main/inst/tutorials/r4ds-4/data/history.parquet`
...
#| echo: true
history_lazy <- open_dataset("data/history.parquet")
history_lazy
```
...
`https://github.com/PPBDS/misc.tutorials/raw/refs/heads/main/inst/tutorials/r4ds-4/data/groups.parquet`
`https://github.com/PPBDS/misc.tutorials/raw/refs/heads/main/inst/tutorials/r4ds-4/data/market_groups.parquet`
...
Convergence tells you markets get more accurate over time, but a deeper question is whether they are well-calibrated — meaning a 70% prediction actually resolves YES 70% of the time. Before testing calibration, get a feel for how the final-day probabilities are distributed. Filter `joined` to snapshots where `days_to_resolution == 0` and plot a histogram of `probability`. Render.
...
The histogram should show a sharp bimodal spike near 0 and 1 — most markets have already converged to near certainty by resolution day. That pattern also tells you where calibration data will be sparse: the middle bins (40–60%) have few markets because genuine uncertainty rarely persists to the final day.

### Exercise 15

Build the calibration table. From the final-day snapshots, bin each market's probability to the nearest 10 percentage points with `round(probability * 10) / 10`, then for each bin compute the fraction that actually resolved YES (`actual_rate`) and the count of markets (`n`). Save the result as `calibration` and print it. Render.
...
The calibration table is the building block of any probabilistic evaluation. For each predicted-probability bin, it records what fraction of events actually happened. If the market is perfectly calibrated, the 0.7 bin's `actual_rate` should be 0.70. Notice that the extreme bins (0 and 1) have the most markets — confirming what the histogram showed.

### Exercise 16

Plot the calibration curve using `calibration`. Put predicted probability on the x-axis, actual resolution rate on the y-axis, and add a dashed 45° reference line to show perfect calibration. Scale both axes as percentages and size each point by `n`, the number of markets in that bin. Render.
...
calibration |>
  ...
  labs(x = "Predicted probability", y = "Actual resolution rate",
       title = "Manifold Markets is well-calibrated",
       subtitle = "Points near the dashed line indicate accurate probability estimates",
       size = "Markets") +
  theme_minimal()
```
...
A well-calibrated forecaster lies on the diagonal: the stated probability matches the long-run frequency of the event. The large dots near 0 and 1 — where most of the data lives — sit close to the reference line, which is the meaningful result. The small dots in the middle (sparse bins with fewer than 25 markets) are noisy; small sample sizes make `actual_rate` unreliable there. Dot size is therefore carrying real information: trust the big dots, treat the small ones skeptically.
