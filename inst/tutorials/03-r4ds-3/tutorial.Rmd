...
About 82% of districts with comparable 2015 and 2023 data show a negative reading score change — the largest single-decade setback in modern US education measurement. The state arrow chart from Exercise 16 puts that aggregate finding into policy context: the exceptions are states that passed evidence-based literacy legislation.
...
The `start_hhmm %/% 100` extraction pulls the hour out of a packed HHMM integer: `1430 %/% 100` is 14, meaning 2 PM. Packed HHMM integers are a common storage trick in government time and transportation data — compact to store, but requiring exactly this kind of arithmetic before use. Filtering to hours 6–23 avoids a before-dawn ambiguity in the ATUS diary format.
